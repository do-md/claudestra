...
Claude Code 是一个只能在终端里用的工具——不在电脑前你就用不了。Claudestra 在你本地的 session 前面搭了一道持久化的门（Discord 和/或 Web 应用），让你可以：
...
- 在 Web 应用里直接打开任意 session 的**实时远程终端**。
...
Discord 之外的第二道前门 —— 完全建立在多前端 API 之上的 **PWA Next.js 应用**。可以和 Discord 并存，也可以完全替代它（Web-only 模式，不需要 bot token）。

- **流式聊天** — 工具调用渲染成三态实时卡片（运行中 / 完成 / 失败），Write 和 Edit 展示语法高亮的 diff，中断和权限 / AskUserQuestion 都是可交互卡片。
- **实时远程终端** — agent tmux 画面的可读写镜像，带移动端控制键条。
- **聊天记录搜索** — 全部 session 全文搜索（live + 归档），全局或单会话内。
- **Skills 面板** — 输入框旁一个按钮浏览并调用所有已发现的 skill；可置顶常用项，其余按使用频率排序。
...
web/                     Next.js Web 客户端（PWA）—— 见 web/SETUP.md + web/CLAUDE.md
master/
  CLAUDE.md.template     大总管行为指令模板（setup 时渲染）
tests/                   315 个用例跨 20 个文件（解析器、路由、注册表、历史……）
install.sh               一键安装脚本
SETUP.md / SETUP.zh-CN.md    完整安装指南（Discord 路线）
web/SETUP.md             Web 客户端安装 + 手机远程访问
docs/                    多前端设计 + Web 前端 API 契约
```
